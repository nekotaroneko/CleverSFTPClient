...
class CleverSFTPClient(ui.View):
	...
	def bt_add_favorite(self, sender):
		if self.connect:
			...
			title = console.input_alert('CleverSFTPClient', 'Title {}'.format(full_path), os.path.basename(full_path), 'OK')
	...
	def sftp_get(self, remote_full_path, local_dir_path, type, local_to_basename=None):
		'''retry_remote_full_pathもチェックしているのでフルパスでも問題ない
		local_to_basenameはtype=fileの時用'''
		remote_basename = os.path.basename(remote_full_path)
		remote_dirname = os.path.dirname(remote_full_path)
		retry_remote_full_path = os.path.join(remote_dirname, remote_basename.strip())
		if type == 'dir':
			try:
				self.ssh.get_all(remote_full_path, local_dir_path, self.progress)
			except Exception as e:
				console.hud_alert(str(e)+' and retrying', 'error')
				
				try:
					self.ssh.get_all(retry_remote_full_path, local_dir_path, self.progress)
				except Exception as e:
					console.hud_alert(str(e), 'error')
					
					
		elif type == 'file':
			if local_to_basename:
				to_path = os.path.join(local_dir_path, local_to_basename)
			else:
				to_path = os.path.join(local_dir_path, remote_basename)
				
			sftp_callback = self.progress(remote_full_path, remote_basename, 'get',open_path=to_path)
			
			try:
				self.sftp.get(remote_full_path, to_path, sftp_callback)
			except Exception as e:
				console.hud_alert(str(e)+' and retrying', 'error')
				
				try:
					self.sftp.get(retry_remote_full_path, to_path, sftp_callback)
				except Exception as e:
					console.hud_alert(str(e), 'error')
					
		elif type == 'link':
			try:
				link_type, link_to = self.get_link_target(remote_basename)
			except Exception as e:
				console.hud_alert('Invalid link?', 'error')
				
				return
			else:
				self.sftp_get(link_to, local_dir_path, link_type)
	...
	def bt_download(self, sender):        #remote
		...
		def download():
			for remote_file in get_table_list(self.tv_remote):
				...
				full_path = remote_dict['full_path']
				basename = remote_file
				_type = remote_dict['type']
				self.sftp_get(full_path, self.local_dirname, _type)
	...
	def table_remote_tapped(self, sender):
		...
		if not self.tv_remote.editing:
			...
			if type == 'dir':
				...
				try:
					link_type, link_to = self.get_link_target(filename_tapped)
				except Exception as e:
					console.hud_alert('Invalid link?', 'error')
				...
				if link_type == 'dir':
					...
					console.hud_alert('{}'.format(link_type))
				return
				
			elif type == 'file':
				result = console.alert("CleverSFTPClient","{}".format(filename_tapped),"Edit with Pythonista editor","Download and open", 'Execute')
				if result == 1:
					local_temp_basename = 'CleverSFTPClient_'+filename_tapped
					to_path= os.path.join(temp_path, local_temp_basename)
					self.sftp_get(full_path, temp_path, 'file', local_temp_basename)
					...
					wait_tab_closed(local_temp_basename)
					...
					self.progress_view.present('sheet')
					self.sftp_get(full_path, download_path, 'file')
	...
	def get_link_target(self, remote_path):
		try:
			link_to = self.sftp.normalize(remote_path)
		except Exception as e:
			raise e
	...
	def progress(self, from_path, to_path, mode, open_path=None):
		def openin_action(sender):
			console.open_in(cell.open_path)
		...
		try:
			...
			width = 534 if ui.get_screen_size()[0]+ui.get_screen_size()[1] > 1500 else ui.get_screen_size()[0]
			cell_label = ui.Label(frame=(0,0,width,44),name="label")			
			...
			openin_btn = ui.Button()
			openin_btn.image = ui.Image.named('iob:ios7_download_outline_256')
			openin_btn.frame = (width-50,0,40,40)
			openin_btn.action = openin_action
			openin_btn.tint_color = "black"
			
			cell.content_view.add_subview(cell_label)
			cell.content_view.add_subview(openin_btn)
